...
Dataset version documented here: **`merged_dataset` v1** (unchanged from
the previously-shipped production dataset — no new version was created,
see "Dataset expansion" below for why). Generated
`2026-08-10`. All figures were computed directly against the files in
...
| Reproducibility | **Verified during this review**: deleting and regenerating this file from `Diseases_and_Symptoms_dataset.csv` via `merge_datasets.py` produces an **MD5-identical** file (`86629c183dd1a580971005452c1a74e7`) to the one already checked in. The pipeline is fully deterministic and reproducible. |
| Record count | 93,993 |
| Feature count | 229 |
| Target classes | 100 |
...
| External sources investigated | Kaggle "Diseases and Symptoms" family (structural candidates only, not downloadable without Kaggle authentication this environment does not have and should not fabricate having); Philippine DOH/PIDSR/EDCS (aggregate surveillance, wrong schema — see §2) |
...
| Train/test feature-vector overlap under the **production** split (`train_test_split`, no grouping) | **1,485 rows** — a real, previously-unaddressed leakage risk: identical symptom presentations appear on both sides of the production train/test boundary |
| Train/test feature-vector overlap under a **group-safe** split (`StratifiedGroupKFold` keyed on feature-vector identity, implemented during this review) | **0 rows** (verified) |
...
| Near-duplicate/label-conflict detection | **Added during this review** | `optimize_random_forest.py: analyze_leakage_and_conflicts()` — was not previously implemented anywhere in the pipeline |
...
| Provenance preservation | **Partial** | The pipeline does not currently tag each merged row with its originating source filename. Since only one source file (§1.1) is actually merged today, this has no practical effect on the current dataset, but is noted as a gap that would need addressing if a second compatible source were ever added. |
...
| Train/test split | Yes | 80/20, stratified, `random_state=42` (production); a leakage-safe grouped alternative was added during this review (§4) |
