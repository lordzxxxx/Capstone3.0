 ...
 *    node set_bho_role.js user@example.com
 *
 * The script will set a custom claim { role: 'BHO' } and write a `role` field
...
async function setBhoRoleByEmail(email) {
  if (!email) throw new Error('Email required. Usage: node set_bho_role.js <email> [--key path]');

  const user = await admin.auth().getUserByEmail(email);
  console.log('Found user:', user.uid, user.email);

  // Set custom claim
  await admin.auth().setCustomUserClaims(user.uid, { role: 'BHO' });
  console.log('Set custom claim role:BHO for', user.uid);

  // Also set Realtime Database users node
  await admin.database().ref(`users/${user.uid}`).update({ role: 'BHO' });
  console.log('Wrote users/{uid}/role = BHO in Realtime Database');
}

initAdmin();
const email = emailArg;
setBhoRoleByEmail(email).then(() => {
  console.log('Done');
  process.exit(0);
}).catch((err) => {
