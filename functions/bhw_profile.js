...
function isApprovedActiveDoctor(profile) {
  const approved = normalizeText(profile?.approvalStatus) === 'approved' ||
    profile?.isApproved === true;
  return normalizeText(profile?.role) === 'doctor' &&
    approved &&
    ['active', 'approved'].includes(
        normalizeText(profile?.accountStatus || profile?.status),
    );
...
function doctorHasProfilePermission(profile) {
  const permissions = Array.isArray(profile?.permissions) &&
    profile.permissions.length
    ? profile.permissions
    : defaultPermissionsForRole(profile?.role || 'DOCTOR');
  return permissions.includes('profile.view');
}
...
// Doctors may update their own professional contact/profile fields, but the
// account role, permissions, assignment eligibility, and status remain CHO
// Admin-controlled fields.
exports.updateOwnDoctorProfile = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
        'unauthenticated',
        'You must be signed in to update your profile.',
  ...
  const userRef = db.collection('users').doc(uid);
  const userSnapshot = await userRef.get();
  if (!userSnapshot.exists || !isApprovedActiveDoctor(userSnapshot.data()) ||
      !doctorHasProfilePermission(userSnapshot.data())) {
    throw new functions.https.HttpsError(
        'permission-denied',
        'Only an approved, active doctor can update this profile.',
    );
  }
  ...
  const professionalTitle = textValue(
      data?.professionalTitle,
      'Professional title',
      {maxLength: 120},
  );
  const specialization = textValue(
      data?.specialization,
      'Specialization',
  ...
  try {
    ...
    console.error('updateOwnDoctorProfile Auth display name sync failed', {
      uid,
      code: error?.code || 'unknown',
    });
  }
  if (!process.env.FUNCTIONS_EMULATOR ||
    ...
    try {
      ...
      // Firestore is authoritative for the portal profile. Keep the legacy
      // Realtime Database mirror best-effort so a mirror outage cannot make a
      // successful profile update look like it failed.
      console.error('updateOwnDoctorProfile RTDB mirror failed', {
        uid,
        code: error?.code || 'unknown',
      });
    }
  ...
  await db.collection('audit_logs').add({
    ...
    targetUid: uid,
    updatedFields: [
      'fullName', 'username', 'contactNumber',
