...
class MortalityAnalyticsPage extends StatefulWidget {
  ...
  @override
  State<MortalityAnalyticsPage> createState() =>
      _MortalityAnalyticsPageState();
}
...
class _MortalityAnalyticsPageState extends State<MortalityAnalyticsPage> {
  ...
  List<Map<String, dynamic>> _demoRecords() {
    ...
    const causes = [
      'Heart Disease', 'Stroke', 'Pneumonia', 'Cancer', 'Diabetes',
    ];
    ...
    return List.generate(72, (index) {
      final date = DateTime(now.year - index % 4, 1 + index % 12, 2 + index % 24);
  ...
  Map<String, int> _ages() {
    final result = <String, int>{
      '<1 year': 0, '1-14': 0, '15-24': 0,
      '25-44': 0, '45-64': 0, '65+': 0, 'Unknown': 0,
    };
    for (final row in _records) {
  ...
  String _category(String cause) {
    ...
    if (value.contains('heart') || value.contains('stroke') || value.contains('cardio')) return 'Cardiovascular';
    if (value.contains('pneumonia') || value.contains('respiratory') || value.contains('lung')) return 'Respiratory';
    if (value.contains('cancer') || value.contains('tumor')) return 'Cancer';
    if (value.contains('infection') || value.contains('sepsis') || value.contains('dengue')) return 'Infectious Diseases';
    if (value.contains('maternal') || value.contains('pregnan')) return 'Maternal';
  ...
  Map<String, int> _riskAssessment() {
    ...
    for (final row in _records) {
      ...
      final category = _category(_v(row, 'causeOfDeath'));
      final label = age >= 65 || category == 'Cardiovascular' || category == 'Cancer'
          ? 'High'
  ...
  Map<String, int> _rates() {
    ...
    for (final row in _records) {
      ...
      if (population != null && population > 0) populations[barangay] = population;
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Mortality Analytics', style: TextStyle(color: _foreground, fontWeight: FontWeight.bold)),
        actions: [
          IconButton(
      ...
      body: _loading
          ...
          : RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  if (_demo) _notice('Showing demo mortality data for analytics.'),
                  _line('Monthly Mortality Trend', 'Deaths recorded during the last 12 months', _monthly()),
                  _gap(),
                  _bar('Leading Causes of Death', 'Most frequently reported causes', _count('causeOfDeath')),
                  _gap(),
                  _bar('Mortality by Age Group', 'Deaths across vulnerable age categories', _ages()),
                  _gap(),
                  _pie('Mortality by Sex', 'Comparison of male and female mortality', _count('gender')),
                  _gap(),
                  _bar('Mortality by Barangay', 'Communities with higher mortality counts', _barangays()),
                  _gap(),
                  _bar('Mortality by Disease Category', 'Broader public-health cause groupings', _categories()),
                  _gap(),
                  _line('Mortality by Year', 'Long-term mortality trend', _years()),
                  _gap(),
                  _pie('Place of Death', 'Where reported deaths occurred', _count('place')),
                  _gap(),
                  _heatmap(),
                  _gap(),
                  _bar('AI Mortality Risk Assessment', 'Population-level planning aid—not an individual death prediction', _riskAssessment()),
                  _gap(),
                  _bar('Mortality Rate per 1,000 Population', 'Rate shown to two decimal places; chart values are ×100', _rates(), valueScale: 0.01),
                ],
              ),
            ),
    );
  ...
  Widget _notice(String message) => Container(
        ...
        child: Text(message, style: const TextStyle(color: _foreground, fontSize: 12, fontWeight: FontWeight.w600)),
      );
  ...
  Widget _bar(String title, String subtitle, Map<String, int> data, {double valueScale = 1}) {
    ...
    return _card(title, subtitle, SizedBox(
      height: 285,
      child: BarChart(BarChartData(
        ...
        titlesData: FlTitlesData(
          ...
          leftTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 34, getTitlesWidget: (value, meta) => Text((value * valueScale).toStringAsFixed(valueScale == 1 ? 0 : 1), style: TextStyle(color: _foreground.withValues(alpha: 0.55), fontSize: 8)))),
        ...
        barTouchData: BarTouchData(touchTooltipData: BarTouchTooltipData(
          ...
          getTooltipItem: (group, groupIndex, rod, rodIndex) {
            ...
            return BarTooltipItem('${entry.key}\n${(entry.value * valueScale).toStringAsFixed(valueScale == 1 ? 0 : 2)}', const TextStyle(color: _foreground, fontWeight: FontWeight.bold));
          },
        )),
  ...
  Widget _heatmap() {
    ...
    return _card('Mortality Heatmap by Barangay', 'Green indicates lower and red indicates higher concentration', entries.isEmpty
        ? _empty()
        : Column(children: entries.map((entry) {
            final color = Color.lerp(const Color(0xFF43A047), const Color(0xFFE53935), entry.value / maximum)!;
            return Container(
              margin: const EdgeInsets.only(bottom: 8),
              padding: const EdgeInsets.all(11),
              decoration: BoxDecoration(color: color.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(9), border: Border.all(color: color.withValues(alpha: 0.45))),
              child: Row(children: [Icon(Icons.location_on_rounded, color: color, size: 18), const SizedBox(width: 8), Expanded(child: Text(entry.key, style: const TextStyle(color: _foreground))), Text('${entry.value}', style: TextStyle(color: color, fontWeight: FontWeight.bold))]),
            );
          }).toList()));
  }
