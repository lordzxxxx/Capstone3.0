...
class CommunicablePage extends StatefulWidget {
  ...
  const CommunicablePage({super.key, this.listOnly = false});
...
class _CommunicablePageState extends State<CommunicablePage> {
  ...
  Future<void> _loadMetrics() async {
    ...
    try {
      ...
      final diseaseTypeCount = <String, int>{};
      for (var record in communicableRecords) {
        final condition = record['condition']?.toString() ?? 'Unknown';
        diseaseTypeCount[condition] = (diseaseTypeCount[condition] ?? 0) + 1;
      }

      setState(() {
        _activeCases = activeCases;
        _diseaseTypes = diseaseTypeCount;
  ...
  Future<void> _seedCommunicableData() async {
    // Show loading dialog
    showDialog(
      context: context,
      ...
      builder: (context) => AlertDialog(
        content: Column(
          ...
          children: const [
            ...
            SizedBox(height: 16),
            Text('Seeding 100 sample communicable disease records...'),
          ],
        ),
      ),
    );
        ...
        actions: [
          ...
          if (widget.listOnly != true)
            PopupMenuButton(
              ...
              itemBuilder: (context) => [
                PopupMenuItem(
                  child: const Text('Seed 100 Sample Records'),
                  onTap: () => _seedCommunicableData(),
                ),
              ],
            ),
        ],
...
class _CommunicableTable extends StatelessWidget {
