...
final Map<String, pw.MemoryImage?> _overallPdfAssetImageCache =
...
Future<Uint8List> _buildOverallHealthPdf(
  ...
  final headerLogos = await Future.wait<pw.MemoryImage?>([
    ...
    _loadPdfAssetImage('assets/logo3.png'),
  ]);
  ...
  pdf.addPage(
    pw.MultiPage(
      ...
      build: (context) => [
        _buildOverallPdfHeader(
          selection,
          leftLogo: headerLogos[0],
          centerLogo: headerLogos[1],
          rightLogo: headerLogos[2],
        ),
...
pw.Widget _buildOverallPdfHeader(
  _OverallReportSelection selection, {
  ...
  final monthLabel = DateFormat.MMMM()
      .format(DateTime(selection.year, selection.month))
      .toUpperCase();

  final barangay = selection.barangay.trim().isEmpty
      ? '________________'
      : selection.barangay.trim();
  final municipality = selection.municipality.trim().isEmpty
      ? '________________'
      : selection.municipality.trim();
  final province = selection.province.trim().isEmpty
      ? '________________'
  ...
  return pw.Container(
    ...
    child: pw.Row(
      ...
      children: [
        // Left side: logos and report info.
        pw.Expanded(
          flex: 3,
          child: pw.Column(
            ...
            children: [
              pw.Row(
                mainAxisSize: pw.MainAxisSize.min,
                children: [
                  _buildOverallHeaderLogoImage(leftLogo),
                  pw.SizedBox(width: 8),
                  _buildOverallHeaderLogoImage(centerLogo),
                  pw.SizedBox(width: 8),
                  _buildOverallHeaderLogoImage(rightLogo),
                ],
              ),
              pw.SizedBox(height: 10),
              pw.Text(
                'FHSIS REPORT for the Month: _${monthLabel}_______ Year: _${selection.year}______',
                style: pw.TextStyle(
                  fontSize: 14,
                  fontWeight: pw.FontWeight.bold,
                  color: PdfColors.black,
              ...
              pw.Text(
                'Name of Municipality/City $municipality',
                style: pw.TextStyle(fontSize: 11, color: PdfColors.black),
              ),
              pw.Text(
                'Name of Province $province',
                style: pw.TextStyle(fontSize: 11, color: PdfColors.black),
              ),
        ...
        // Right side: M1BRGY branding.
        pw.Container(
          padding: const pw.EdgeInsets.only(bottom: 72, right: 10),
          decoration: pw.BoxDecoration(),
          child: pw.Text(
            'M1BRGY',
            style: pw.TextStyle(
...
pw.Widget _buildOverallHeaderLogoImage(pw.MemoryImage? image) {
  return pw.SizedBox(
    width: 72,
    height: 72,
    child: image == null
...
Future<pw.MemoryImage?> _loadPdfAssetImage(String assetPath) async {
  if (_overallPdfAssetImageCache.containsKey(assetPath)) {
    return _overallPdfAssetImageCache[assetPath];
  }

  try {
    final byteData = await rootBundle.load(assetPath);
    final optimizedBytes = await _optimizeOverallPdfImageBytes(
      byteData.buffer.asUint8List(),
      targetLongestSide: 260,
    );
    final image = pw.MemoryImage(optimizedBytes);
    _overallPdfAssetImageCache[assetPath] = image;
    return image;
  } catch (_) {
    _overallPdfAssetImageCache[assetPath] = null;
    return null;
  }
}

Future<Uint8List> _optimizeOverallPdfImageBytes(
  Uint8List bytes, {
...
pw.Widget _buildHeaderDetailCompact(String label, String value) {
  return pw.Padding(
